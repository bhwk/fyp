...
def extract_responses(data):
    """Extract only the 'synthesized information' and 'response' fields, filtering out empty responses."""
    filtered_data = [item for item in data if item.get("response")]
    synthesized_information = [
        item["synthesized information"] for item in filtered_data
    ]
    responses = [item["response"] for item in filtered_data]
    return synthesized_information, responses
